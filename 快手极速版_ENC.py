...
#   * 可选变量(可不填): lindong_ks_maxgold    最大金币数        默认 500000 (跑多了第二天可能1金币 有此需求请勿兑换金币)
#   * 可选变量(可不填): lindong_ks_run_task   额外执行任务      默认 无 多个额外执行的任务用,分割 填0,1,2,3,4 执行所有任务
#   * 可选的是 0:签到 | 1.宝箱 | 2.宝箱广告(需要运行宝箱广告必须开宝箱) | 3.饭补广告 | 4.搜索广告
#   * 可选变量(可不填): lindong_ksyhtimes     执行多少个广告后进行休眠操作     默认 40
#   * 可选变量(可不填): lindong_autotixi      是否自动提现       默认否 填1自动提现
#   * 可选变量(可不填): lindong_tiximoney     提现金额           默认50 可选50或20或10
...
#   * 可选变量(可不填): lindong_searchKeyWord 搜索广告的关键词    默认为 '短剧小说'
